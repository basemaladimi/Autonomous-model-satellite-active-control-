...
#define UART_TEST_NODE DT_NODELABEL(uart2)
#define N 5
...
static const struct device *uart_dev = DEVICE_DT_GET(UART_TEST_NODE);
float A_inv[3][3] = {
...
int main(void) {
		...
		while (1) {
				...
				if (baseline_locked) {
					...
					int len = snprintk(tx_buf, sizeof(tx_buf), "%.6f,%.6f,%.6f,%.6f,%.6f\n",
									pitchc, rollc, AltitudeBarometer, t, p_used);
